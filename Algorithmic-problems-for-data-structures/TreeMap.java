 ...
 * Implements SortedMap ADT functions: firstEntry(), lastEntry(), ceilingEntry(k),
 * floorEntry(k), lowerEntry(k), higherEntry(k), subMap(k1, k2)
 * Implementing Map  functions: size(), get(k), put(k, v), remove(k), entrySet()
 * Additional public methods: toString(), toStringTree() - using BinaryTreePrinter
 * Map functions: isEmpty(), keySet(), values() are already implemented in
...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
		...
		public void rotate(Position<Entry<K, V>> p) {
			Node<Entry<K,V>> up = validate(p);
			Node<Entry<K,V>> down = up.getParent();
			Node<Entry<K,V>> straight = down.getParent();
			if (straight != null) {
				relink(straight, up, !(down != straight.getLeft()));
			} else {
				root = up;
				up.setParent(null);
			}
			if (up.equals(down.getLeft())) {
				relink(down, up.getRight(), true);
				relink(up, down, false);
			} else {
				relink(down, up.getLeft(), false);
				relink(up, down, true);
			}
		}
		...
		public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
			...
			if (!((x.equals(right(y))) == (y.equals(right(z))))) {
				for (int i = 0; i < 2; i++) {
					rotate(x);
				}
				return x;
	...
	public V get(K key) throws IllegalArgumentException {
		if (!isExternal(treeSearch(root(), key)))
	...
	public V remove(K key) throws IllegalArgumentException {
		...
		Position<Entry<K, V>> p = treeSearch(root(), key);
		if (!isExternal(p)) {
			V removee = p.getElement().getValue();
			if (!isInternal(left(p))) {
			} else if (!isInternal(right(p))) {
			} else {
				Position<Entry<K,V>> right = treeMax(p);
				set(p,right.getElement());
				p = right;
			}
			Position<Entry<K, V>> leaf;
			if (isExternal(left(p))) leaf = left(p);
			else leaf = right(p);
	...
	public Iterable<Entry<K, V>> entrySet() {
		...
		for (Iterator<Position<Entry<K, V>>> iterator = tree.inorder().iterator(); iterator.hasNext(); ) {
			Position<Entry<K, V>> p = iterator.next();
			if (isInternal(p)) {
	...
	// getting th representation of th string
	...
	public Iterable<Entry<K, V>> subMap(K fromKey, K toKey) throws IllegalArgumentException {
		ArrayList<Entry<K, V>> store = new ArrayList<>(size());
		if (compare(fromKey, toKey) < 0) {
			RSubMap(fromKey, toKey, root(), store);
		}
		return store;
	}
	// recursively filling the entries in the given range
	private void RSubMap(K fromKey, K toKey, Position<Entry<K, V>> p, ArrayList<Entry<K, V>> store) {
		if (isInternal(p)) {
			if (compare(p.getElement(), fromKey) < 0) {
				RSubMap(fromKey, toKey, right(p), store);
			} else {
				RSubMap(fromKey, toKey, left(p), store);
				if (compare(p.getElement(), toKey) < 0) {
					store.add(p.getElement());
					RSubMap(fromKey, toKey, right(p), store);
				}
			}
		}
	...
	// main method to test whether the functions are working properly
