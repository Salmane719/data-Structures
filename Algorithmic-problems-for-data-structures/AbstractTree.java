...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public int numChildren(Position<E> p) {
        // TODO
        int count = 0 ;
        for (Iterator<Position<E>> iterator = children(p).iterator(); iterator.hasNext(); ) {
            Position child = iterator.next();
            count++;
        }
        return count;
    }
    ...
    public int size() {
        // TODO
        int count = 0;
        for (Iterator<Position<E>> iterator = positions().iterator(); iterator.hasNext(); ) {
    ...
    public int height(Position<E> p) throws IllegalArgumentException {
        int height =  0 ;
        for (Iterator<Position<E>> iterator = children(p).iterator(); iterator.hasNext(); ) {
            Position<E> l = iterator.next();
            height = Math.max(height, 1 + height(l));
        }
    ...
    public Iterable<Position<E>> preorder() {
        List<Position<E>> snapchot = new ArrayList<>();
        if (!isEmpty())
            preorderSubtree(root(),snapchot);
        return snapchot;
    }
